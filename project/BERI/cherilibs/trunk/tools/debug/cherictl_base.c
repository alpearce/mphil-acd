...
print_trace_entry(struct beri_debug_trace_entry *tep)
{
  ...
  static uint64_t global_instruction_count = 0;
	...
	if (tep->branch) printf(" branch to 0x%16.16jx", tep->val1);
	switch (tep->version) {
  case 0:
	  printf(" {%d}\n", tep->asid);
	  break;
  ...
  case 3:
    ...
    printf("  CPI %16.16f\n", (double)(tep->val1 - global_cycle_count)/
	  ...
	  break;
	case 11:
	  printf("  CapReg <- tag:%1jx u:%1jx perms:0x%8.8jx type:0x%6.6jx offset:0x%16.16jx base:0x%16.16jx length:0x%16.16jx {%d}\n", 
	  	(tep->val2>>63) & 0x1,
	  	(tep->val2>>62) & 0x1,
	  	(tep->val2>>53) & 0xFF,
	  ...
	  break;
	case 12:
	  printf("  CapReg <- tag:%1jx u:%1jx perms:0x%8.8jx type:0x%6.6jx offset:0x%16.16jx base:0x%16.16jx length:0x%16.16jx from Address 0x%16.16jx {%d}\n",
	    (tep->val2>>63) & 0x1,
	  	(tep->val2>>62) & 0x1,
	  	(tep->val2>>53) & 0xFF,
	  	(tep->val2>>32) & 0x3FFFFF,
	  ...
	  break;
  case 13:
	  printf("  Address 0x%16.16jx <- tag:%1jx u:%1jx perms:0x%8.8jx type:0x%6.6jx offset:0x%16.16jx base:0x%16.16jx length:0x%16.16jx {%d}\n",
	    tep->val1, 
	    (tep->val2>>63) & 0x1,
			(tep->val2>>62) & 0x1,
			(tep->val2>>53) & 0xFF,
